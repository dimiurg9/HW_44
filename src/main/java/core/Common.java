...
class Common {
	...
	static void getWebDriver(String browser){
		...
		driver.manage().timeouts().implicitlyWait(15, TimeUnit.SECONDS);
			...
			if (System.getProperty("os.name").toUpperCase().contains("MAC"))
			driverPath = "/usr/local/bin/chromedriver.sh";
			else if (System.getProperty("os.name").toUpperCase().contains("WINDOWS"))
			driverPath = "/usr/local/bin/chromedriver.sh";
			else throw new IllegalArgumentException("Unknown OS");
//			System.setProperty("webdriver.chrome.driver", driverPath);
			...
			if (System.getProperty("os.name").toUpperCase().contains("MAC"))
				...
				driverPath = "\\resources\\webdrivers/pc/path_rof_windows";
				else throw new IllegalArgumentException("Unknown OS");
	...
	static String getValue(By by) {
		...
		return driver.findElement(by).getAttribute("value").toString().trim().replace(", ", ".");
